...
import org.example.Others.Purchase;
...
public class SellsController implements Initializable{
    ...
    /**
     * Generates line chart for displaying sales data.
     */
    ...
    /**
     * Handles the action event for displaying the bar chart or table.
    ...
    /**
     * Handles the action event for reloading data.
     * @param event The action event.
     */
    @FXML
    ...
    /**
     * Handles the action event for proceeding with the transaction.
     * @param event The action event.
     */
    @FXML
    void btnProceedAction(ActionEvent event) {
        if (startTransaction) {
            ...
            if (!txtQty.getText().equals("") && Integer.parseInt(txtQty.getText()) > 0) {
                ...
                //final JFXSnackbar.SnackbarEvent snackbarEvent = new JFXSnackbar.SnackbarEvent(new Label("Quantity field can not be null or 0"), Duration.millis(3000), null);
                //snackbar.enqueue(snackbarEvent);
            }
